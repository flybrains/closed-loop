...
def run_fictrac_client(LOCAL_HOST, LOCAL_PORT, RPI_HOST, RPI_PORT):
	...
	if config.PLAYBACK:
		hdlr = logging.FileHandler(r'C:\Users\rutalab\Desktop\fictrac-data\client-logs\PLAYBACK{}.log'.format(dts))
		...
		logger.addHandler(hdlr)
		logger.setLevel(logging.DEBUG)
		logger.info("{} -- {},{},{},{},{},{},{}".format("timestamp","mfc1_stpt","mfc2_stpt","mfc3_stpt", "mfc1_device_output", "mfc2_device_output", "mfc3_device_output", "PID_read"))
		...
		try:
			...
			while True:
				...
				if config.PLAYBACK:
					...
					target = 800000
					print(mfc1_sp, mfc2_sp, mfc3_sp)
					# SENDSTRING = '<'+ '{},{},{},{},{},{},{}'.format(1,target, mfc1_sp, mfc2_sp, mfc3_sp, 0.0, 0.0) +'>\n'
					# RPI_SOCK.sendto(str.encode('{}'.format(SENDSTRING)), ("192.168.137.10", 5000))
					# dts = datetime.now().strftime("%m/%d/%Y-%H:%M:%S.%f")
					# logger.info("{} -- {},{},{},{},{},{},{}".format(dts,mfc1_sp,mfc2_sp,mfc3_sp, "mfc1_device_output", "mfc2_device_output", "mfc3_device_output", "PID_read"))
